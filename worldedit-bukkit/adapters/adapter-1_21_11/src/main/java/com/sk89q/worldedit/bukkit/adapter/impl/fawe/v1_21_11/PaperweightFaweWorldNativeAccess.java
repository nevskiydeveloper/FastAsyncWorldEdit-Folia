...
import com.fastasyncworldedit.bukkit.util.BukkitSchedulerCompat;
...
public class PaperweightFaweWorldNativeAccess implements WorldNativeAccess<LevelChunk,
    ...
    public PaperweightFaweWorldNativeAccess(PaperweightFaweAdapter paperweightFaweAdapter, WeakReference<Level> level) {
        ...
        this.lastTick = new AtomicInteger(currentTickSnapshot());
    }
    ...
    public synchronized net.minecraft.world.level.block.state.BlockState setBlockState(
        ...
        int currentTick = currentTickSnapshot();
        if (BukkitSchedulerCompat.isOwnedByCurrentRegion(getLevel().getWorld(), levelChunk.locX, levelChunk.locZ)) {
    ...
    private synchronized void flushAsync(final boolean sendChunks) {
        ...
        applyQueuedChanges(changes, toSend, false);
    }

    @Override
    public synchronized void flush() {
        applyQueuedChanges(Set.copyOf(cachedChanges), Set.copyOf(cachedChunksToSend), true);
        cachedChanges.clear();
        cachedChunksToSend.clear();
    }

    private void applyQueuedChanges(Set<CachedChange> changes, Set<IntPair> chunksToSend, boolean wait) {
        if (changes.isEmpty() && chunksToSend.isEmpty()) {
            return;
        }
        CraftWorld bukkitWorld = getLevel().getWorld();
        Map<IntPair, List<CachedChange>> changesByChunk = new HashMap<>();
        for (CachedChange change : changes) {
            IntPair pair = new IntPair(change.levelChunk.locX, change.levelChunk.locZ);
            changesByChunk.computeIfAbsent(pair, ignored -> new ArrayList<>()).add(change);
        }
        Set<IntPair> allChunks = new HashSet<>(changesByChunk.keySet());
        allChunks.addAll(chunksToSend);
        List<CompletableFuture<Void>> futures = new ArrayList<>(allChunks.size());
        boolean updateNeighbors = sideEffectSet.shouldApply(SideEffect.UPDATE);
        for (IntPair chunk : allChunks) {
            List<CachedChange> queuedChanges = changesByChunk.getOrDefault(chunk, Collections.emptyList());
            boolean sendChunk = chunksToSend.contains(chunk);
            futures.add(BukkitSchedulerCompat.runAtChunk(
                    WorldEditPlugin.getInstance(),
                    bukkitWorld,
                    chunk.x(),
                    chunk.z(),
                    () -> {
                        for (CachedChange change : queuedChanges) {
                            change.levelChunk.setBlockState(change.blockPos, change.blockState, updateNeighbors ? 0 : 512);
                        }
                        if (sendChunk) {
                            PaperweightPlatformAdapter.sendChunk(chunk, bukkitWorld.getHandle(), chunk.x(), chunk.z());
                        }
                    }
            ));
        }
        if (wait && !futures.isEmpty()) {
    ...
    private static int currentTickSnapshot() {
        try {
            return Bukkit.getCurrentTick();
        } catch (IllegalStateException ignored) {
            return (int) (TimeUnit.NANOSECONDS.toMillis(System.nanoTime()) / 50L);
        }
    }
