...
public class BukkitWorld extends AbstractWorld {
    ...
    public boolean clearContainerBlockContents(BlockVector3 pt) {
        ...
        testCoords(pt);
        //FAWE end
        return BukkitSchedulerCompat.callAtChunk(
                ...
                () -> {
                    ...
                    if (chest instanceof Chest) {
                        inventory = ((Chest) chest).getBlockInventory();
                    }
                    inventory.clear();
                    return true;
                }
    ...
    public void checkLoadedChunk(BlockVector3 pt) {
        ...
        int X = pt.x() >> 4;
        int Z = pt.z() >> 4;
        if (BukkitSchedulerCompat.isOwnedByCurrentRegion(world, X, Z)) {
            world.getChunkAt(X, Z);
        } else if (PaperLib.isPaper()) {
            PaperLib.getChunkAtAsync(world, X, Z, true);
        } else {
            BukkitSchedulerCompat.callAtChunk(WorldEditPlugin.getInstance(), world, X, Z, () -> {
                world.getChunkAt(X, Z);
                return null;
            });
        }
    ...
    public boolean playEffect(Vector3 position, int type, int data) {
        ...
        BukkitSchedulerCompat.executeAtLocation(WorldEditPlugin.getInstance(), BukkitAdapter.adapt(world, position),
    ...
    public boolean playBlockBreakEffect(Vector3 position, BlockType type) {
        World world = getWorld();
        BukkitSchedulerCompat.executeAtLocation(
                WorldEditPlugin.getInstance(),
