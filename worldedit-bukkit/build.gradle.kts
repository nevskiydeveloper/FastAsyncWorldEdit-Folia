...
val rootJarOutputDir = layout.dir(provider {
    File("G:\\Data\\Minecraft Development\\Allync Project\\Folia Plugin Fork\\plugin-jar")
})
...
tasks.register<ShadowJar>("reobfShadowJar") {
    ...
    archiveFileName.set("${rootProject.name}-Folia-Compat-${project.version}.${archiveExtension.getOrElse("jar")}")
    destinationDirectory.set(rootJarOutputDir)
...
tasks.named<ShadowJar>("shadowJar") {
    archiveFileName.set("${rootProject.name}-Folia-${project.version}.${archiveExtension.getOrElse("jar")}")
    destinationDirectory.set(rootJarOutputDir)
